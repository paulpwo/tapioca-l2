...
import 'video_editor/video_trimmer/video_trim_screen.dart';
...
class _MyAppState extends State<MyApp> {
  ...
  PickedFile? _video = PickedFile("/data/user/0/me.anharu.video_editor_example/cache/image_picker6730465733272534646.mp4");
  ...
  void _processVideo() async {
    try {
      print("clicked!");
      navigatorKey.currentState?.push(MaterialPageRoute(builder: (_) => VideoScreen(_video!.path)));
      return;
      // await _pickVideo();
  ...
  Future<Uint8List?> getImage() async {
    ...
    print("getImage called boundary ${_repaintBoundaryKey.currentContext?.findRenderObject() is RenderRepaintBoundary} ");
            ...
            child: isLoading
                ...
                : Column(
                    children: [
                      ...
                      ElevatedButton(
                        child: Text("video and Edit it"),
                        onPressed: _processVideo,
                      ),
                      ElevatedButton(
                          onPressed: () {
                            navigatorKey.currentState?.push(MaterialPageRoute(builder: (_) => VideoTrimScreen(_video!.path)));
                          },
